...
// Страница мессенджера каждого аккаунта Wazzup. Пути не вложены друг в друга:
// подсветка пункта меню сравнивает начало пути.
export const MESSENGER_PATHS: Record<WazzupAccount, string> = {
  main: "/whatsapp",
  child: "/whatsapp-child",
};
...
const getWidgetErrorMessage = (error: any) => {
  ...
  if (code === "wazzup_account_not_connected") {
    return "Этот аккаунт Wazzup ещё не подключён. Администратор подключает его в разделе «Настройки → Каналы мессенджера».";
  }
  if (code === "wazzup_account_not_configured") {
    return "Этот аккаунт Wazzup не настроен на сервере. Обратитесь к администратору.";
  }
...
export function MessengerView({ account }: { account: WazzupAccount }) {
  ...
  useEffect(() => {
    ...
    getWazzupChannels()
      .then((res) => {
        // Писать можно только с номеров этого аккаунта: чужой аккаунт о них не знает.
        const list = (res?.value || []).filter((c) => (c.account || "main") === account);
  ...
  const loadWidget = useCallback(async () => {
    ...
    try {
      const response = await getWazzupIframe(
        chatId ? { account, transport: chatTransport || "whatsapp", chat_id: chatId } : { account }
      );
  ...
  }, [account, chatId, chatTransport, router, searchQuery]);
  ...
  const handleSetup = async () => {
    ...
    try {
      await setupWazzup({ ...setupForm, account });
